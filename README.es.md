...
> Todas las imágenes siguientes usan **datos sintéticos y ficticios** (un tenant de ejemplo “Contoso”), solo con fines ilustrativos — ninguna información real de tenant, suscripción o recurso.

### Informe HTML Ejecutivo

Informe completo con menú lateral izquierdo, KPIs, gráficos y la sección Cloud Modernization Signals & Opportunity:

![Informe Ejecutivo — página completa](docs/images/executive-full.png)

Cloud Modernization Signals & Opportunity — gráficos As-Is + medidor de preparación y tarjetas de oportunidad:

![Ejecutivo — Modernization Signals & Opportunity](docs/images/executive-modernization.png)

### Informe HTML Técnico

Menú lateral izquierdo con la sección Modernization Signals (As-Is + Opportunities):

![Informe Técnico — sidebar + Modernization](docs/images/technical-modernization.png)

### Inventario Excel

Panel Overview — KPIs, Service Model, Business Pillar, Modernization Signals y posture de planes de Defender for Cloud:

![Excel — Overview](docs/images/excel-overview.png)

Taxonomía de clasificación de recursos (Categoría Técnica / Pilar de Negocio / Modelo de Servicio / Publisher):

![Excel — Classification](docs/images/excel-classification.png)

Tabla plana `AllResources`:
...
Network Topology — VNets / subnets / peerings, con detección de huérfanos:

![draw.io — Network Topology](docs/images/drawio-network-topology.png)

Network Detail — recursos ubicados dentro de sus subnets:
...
- **Clasificación granular de recursos** — los recursos se clasifican con refinamiento por sub-namespace. La Categoría Técnica aparece en Excel (`AllResources`, `Classification`, `ModernizationSignals` y `ResiliencyEvidence`) y en el informe Technical HTML.
- **Informes HTML ejecutivo y técnico** — el Executive HTML incluye **Executive Evidence Summary** factual, perfil por pilar WAF, KPIs de postura de planes Defender, resiliencia y señales de modernización claramente marcadas como `INFERRED`. El Technical HTML organiza WAF, Zero Trust, Defender posture, policy records, misconfiguraciones, health, deprecated resources y Technical Category Distribution. Tablas/tarjetas grandes soportan `Load More` y `Show less`.
...
## Manejo de Datos del Cliente y Aviso de Uso

ATI es un acelerador open source de evaluación de solo lectura. No es un producto oficial de Microsoft, una certificación de cumplimiento ni un sustituto de una revisión formal de arquitectura, seguridad, finanzas, regulación o aspectos legales. ATI no modifica recursos de Azure ni realiza remediación automática.

ATI se ejecuta con una identidad, permisos de Azure y alcance seleccionados y controlados por el cliente. Los informes Excel, HTML y draw.io se guardan únicamente en el directorio local de salida o destino de almacenamiento elegido para la ejecución. ATI no carga ni transfiere el contenido de los informes generados a otro lugar. Como los resultados pueden contener metadatos sensibles del entorno Azure, el cliente debe aprobar su almacenamiento, acceso, retención y distribución conforme a sus políticas aplicables.
